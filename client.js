...
const els = Object.fromEntries([
  ...
  "fullName","idNumber","documentType","documentNumberLabel","country","nationality","sex","dob","placeOfBirth","issueDate","expiry","issuingAuthority","address","verificationNotes","photoUrl","photoFile","photoFormPreview",
...
function renderRecord(record){
  if(!record){
    ...
    document.querySelector(".issued-card")?.classList.remove("is-foreign-id");
  ...
  if(els.previewName){
    const isForeignId = ["foreign id card", "national id card"].includes(String(record.documentType || "").toLowerCase());
    const issuedCard = document.querySelector(".issued-card");
    issuedCard?.classList.toggle("is-foreign-id", isForeignId);
...
function getRecordOutcome(record){
  ...
  const rawDocumentType = String(record?.documentType || "Passport");
  const isNationalId = ["foreign id card", "national id card"].includes(rawDocumentType.toLowerCase());
  const documentType = isNationalId ? "National ID Card" : rawDocumentType;
  const isPassport = documentType.toLowerCase().includes("passport");
  const validLabel = isPassport ? "Valid passport" : `Valid ${documentType}`;
  return { label: validLabel, tone:"valid", title:"Record verified", message:"This reference matches an approved record and is currently within its stated validity period." };
}
function showResult(record){
  ...
  setText(els.resultDocument, ["foreign id card", "national id card"].includes(String(record.documentType || "").toLowerCase()) ? "National ID Card" : record.documentType); setText(els.resultStatus, outcome.label);
...
function downloadVerificationSummary(record){
  if(!record) return;
  const outcome=getRecordOutcome(record);
  const isNationalId = ["foreign id card", "national id card"].includes(String(record.documentType || "").toLowerCase());
  const displayDocumentType = isNationalId ? "National ID Card" : String(record.documentType || "Passport");
...
function updateDocumentTypeFields(){
  if(!els.documentType || !els.idNumber) return;
  const isForeignId = els.documentType.value === "National ID Card";
  if(els.documentNumberLabel) els.documentNumberLabel.textContent = isForeignId ? "ID card / document number" : "Passport number";
  els.idNumber.placeholder = isForeignId ? "ID card or document number" : "Passport number";
  if(els.identitySectionTitle) els.identitySectionTitle.textContent = isForeignId ? "National ID Card information" : "Identity information";
  if(els.identitySectionHelp) els.identitySectionHelp.textContent = isForeignId ? "Enter the details exactly as verified from the national ID card." : "Enter the details exactly as verified from the submitted document.";
}

function renderAdminActions(){
  if(!isAdminPage && !isAdminDashboard) return;
  els.loginForm?.addEventListener("submit",login); els.logoutBtn?.addEventListener("click",logout); els.documentType?.addEventListener("change",updateDocumentTypeFields); updateDocumentTypeFields();
