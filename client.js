...
function renderRecord(record){
  ...
  if(els.previewName){
    const isForeignId = String(record.documentType || "").toLowerCase() === "foreign id card";
...
function getRecordOutcome(record){
  ...
  const documentType = String(record?.documentType || "Passport");
...
function updateDocumentTypeFields(){
  ...
  const isForeignId = els.documentType.value === "Foreign ID Card";
  ...
  if(els.identitySectionTitle) els.identitySectionTitle.textContent = isForeignId ? "Foreign ID Card information" : "Identity information";
  if(els.identitySectionHelp) els.identitySectionHelp.textContent = isForeignId ? "Enter the details exactly as verified from the foreign ID card." : "Enter the details exactly as verified from the submitted document.";
}
